...
useRouteMatch is a hook for react-router, that is used to connect or match a url instead of having to render a route, it's often used if there are nested routes.

1. How would you explain form validation to someone who has never programmed before?

Form validation is put in place so the server can check that the data entered is what is wanted and needed. For example, to make sure that a name is entered and it is long enough to be a legit name, you would specify that in the form validation, by stating that name is required and that it has a minimum, etc.

1. In 1-2 sentences, define end to end testing.

It is a method that tests wether an application can be used in a real-life scenario, and how the user would interact with the application. What errors they might make, what they would look at first, etc.
